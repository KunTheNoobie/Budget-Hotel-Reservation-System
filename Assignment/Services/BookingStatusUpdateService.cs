...
namespace Assignment.Services
{
    ...
    public class BookingStatusUpdateService
    {
        ...
        public async Task<int> UpdateBookingStatusesAsync()
        {
            ...
            try
            {
                // 1. Auto Check-in: Confirmed bookings where check-in date has arrived
                ...
                foreach (var booking in bookingsToCheckIn)
                {
                    booking.Status = BookingStatus.CheckedIn;
                    updatedCount++;
                    _logger.LogInformation("Auto check-in: Booking {BookingId} status changed to CheckedIn", booking.BookingId);
                }
